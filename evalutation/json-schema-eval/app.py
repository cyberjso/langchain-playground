...
DATASET_NAME = "json-schema-code-review"
MODEL_NAME = "gpt-4"
TEMPERATURE = 0
...
LangChainStringEvaluator("json_validity", prepare_data = None)
...
def __validate_schema(run, esx):
    try:
        data = loads(run.output.get("output"))
...
dataset_client = LangSmithClient()
dataset_preparation_report  =  prepare_dataset(dataset_name = DATASET_NAME, 
...
#                   data = DATASET_NAME, 
#                   evaluators = [__validate_schema, LangChainStringEvaluator("json-validation", prepare_data = None)], 
